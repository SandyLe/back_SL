<template>
  <div>
    ...
    <Modal v-draggable="options" title="新增图册" v-model="addModalVisible" @on-ok="addData" @on-cancel="cancel"
           :loading='loading'>
...
export default {
              ...
              return h('Button', {
                on: {
                  'click': () => {
                    ...
                    getOneData('album', params.row.id).then(res => {
  ...
  methods: {
    ...
    addData () {
      var albumObj = JSON.parse(JSON.stringify(this.form_obj))
      saveData('album', albumObj).then((res) => {
        alert('操作成功')
        this.reload()
      })
    },
