<template>
  <div>
    ...
    <Modal v-draggable="options" title="新增产品" v-model="addModalVisible" @on-ok="addData" @on-cancel="cancel"
           :loading='loading'>
      <Form ref="saveForm" :model="form_obj">
        ...
        <FormItem>
          <label for="name" class="ivu-form-label-left lableFormField">店铺地址：</label>
          <input type="text" class="ivu-input inputFormField" name="form_obj.shopUrl" v-model="form_obj.shopUrl"/>
        </FormItem>
        <FormItem>
...
export default {
          ...
          button: [
            (h, params, vm) => {
              return h('Button', {
                on: {
                  'click': () => {
                    ...
                    getOneData('product', params.row.id).then(res => {
              ...
              return h('Poptip', {
                ...
                on: {
                  'on-ok': () => {
                    deleteData('product', params.row.id).then(res => {
                    })
  ...
  methods: {
    ...
    addData () {
      var productObj = JSON.parse(JSON.stringify(this.form_obj))
      saveData('product', productObj).then((res) => {
        alert('操作成功')
        this.reload()
      })
    },
