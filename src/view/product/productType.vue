<template>
  <div>
    ...
    <Modal v-draggable="options" title="新增产品类型" v-model="addModalVisible" @on-ok="addData" @on-cancel="cancel"
           :loading='loading'>
...
export default {
          ...
          button: [
            (h, params, vm) => {
              return h('Button', {
                on: {
                  'click': () => {
                    ...
                    getOneData('productType', params.row.id).then(res => {
              ...
              return h('Poptip', {
                ...
                on: {
                  'on-ok': () => {
                    deleteData('productType', params.row.id).then(res => {
                    })
  ...
  methods: {
    ...
    addData () {
      var productTypeObj = JSON.parse(JSON.stringify(this.form_obj))
      saveData('productType', productTypeObj).then((res) => {
        alert('操作成功')
        this.reload()
      })
    },
